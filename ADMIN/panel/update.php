...
$ip=$_SERVER['REMOTE_ADDR'];
 ...
 $valued=$_POST['valued'];
  ...
  country=:country,ballance=:ballance,profit=:profit,bonus=:bonus,plan=:plan,date=:date,valuei=:valuei,valued=:valued WHERE id=:id");
  $statement->bindValue(':email',$email);
  $statement->bindValue(':fname',$fname);
  $statement->bindValue(':lname',$lname);
  $statement->bindValue(':date',$date);
...
<?php if(!empty($error)):?>
  ...
  <div class="form-group">
    <label>date it will proceed</label>
    <input type="date" name='valued' class="form-control" value="<?php echo $valued?>">
  </div>
